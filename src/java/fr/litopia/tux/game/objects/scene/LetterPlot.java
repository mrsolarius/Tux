...
public class LetterPlot implements PhysicsCollisionListener {
    ...
    public LetterPlot(GameFindWord context, char exceptedLetter, int wordPosition, float x, float z, float size, float rotation){
        ...
        if(count<=20)
            mat_plot.setTexture("ColorMap",context.getAssetManager().loadTexture("assets/textures/plot/brick"+count+".png"));
        else
            mat_plot.setTexture("ColorMap",context.getAssetManager().loadTexture("assets/textures/plot/brick.png"));
